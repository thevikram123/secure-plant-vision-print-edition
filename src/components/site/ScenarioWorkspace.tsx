...
export function ScenarioWorkspace() {
  ...
  return (
    <div className="overflow-hidden rounded-[2rem] border border-navy/10 bg-white shadow-card">
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-navy/10 bg-[#fffdf8] px-5 py-4">
        ...
        <div className="flex items-center gap-2 rounded-full border border-navy/10 bg-white px-3 py-1.5">
          ...
          <span className="text-[0.6rem] font-bold uppercase tracking-[0.12em] text-navy/50">
            Case {String(current.id).padStart(2, "0")} of {useCases.length}
          </span>
        </div>
      </div>
      ...
      <div className="grid min-h-[43rem] lg:grid-cols-[18rem_1fr]">
        <aside className="border-b border-navy/10 bg-[#f3efe5] lg:border-b-0 lg:border-r">
          ...
          <div data-presentation-scroll className="max-h-[38rem] overflow-y-auto px-3 py-3">
              ...
              return (
                <div key={group} className="mb-5 last:mb-0">
                  <div className="mb-2 flex items-center justify-between gap-3 px-2">
                    ...
                    <span className="font-mono text-[0.58rem] text-navy/35">{scenarios.length}</span>
                  </div>
                  <div className="space-y-1">
                    {scenarios.map((scenario) => (
                      ...
                      >
                        ...
                        <span className="text-xs font-semibold leading-snug">{scenario.name}</span>
                      </button>
                    ))}
                  </div>
                </div>
...
function CaseHero({ scenario }: { scenario: UseCase }) {
  return (
    <div className="relative min-h-[14rem] overflow-hidden bg-[#eee9dc]">
      <img
        key={scenario.group}
      ...
      <div className="relative flex min-h-[14rem] max-w-xl flex-col justify-end p-6">
        ...
        <p className="mt-2 text-sm font-medium text-navy/55">{scenario.group}</p>
      </div>
    </div>
  );
}
...
function CaseChain({ scenario }: { scenario: UseCase }) {
  ...
  return (
    <div className="grid gap-2 border-t border-navy/10 bg-[#fffdf8] p-4 sm:grid-cols-[1fr_auto_1fr_auto_1fr] sm:items-center">
          ...
          <div className="rounded-2xl border border-navy/10 bg-white p-4">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="text-[0.58rem] font-bold uppercase tracking-[0.14em] text-navy/40">{label}</p>
                <p className="mt-1 text-xs font-semibold leading-snug text-navy">{value}</p>
              </div>
            </div>
          </div>
          {index < chain.length - 1 ? <ArrowRight className="hidden h-4 w-4 text-navy/25 sm:block" /> : null}
        </div>
      ))}
    </div>
  );
...
function CaseText({ label, body, icon: Icon }: { label: string; body: string; icon: typeof ScanSearch }) {
  return (
        ...
        {scenario.response.map((step, index) => (
          <li key={step} className="grid grid-cols-[2rem_1fr] gap-3">
            <div className="flex flex-col items-center">
              ...
              {index < scenario.response.length - 1 ? <span className="h-7 w-px bg-navy/15" /> : null}
            </div>
            <div className="pt-1.5">
              <p className="text-sm font-semibold leading-snug text-navy">{step}</p>
              {index === scenario.response.length - 1 ? (
                <div className="mt-2 flex items-center gap-2 text-[0.6rem] font-bold uppercase tracking-[0.12em] text-navy/40">
